...
const bit<11> MinTh = 14;
const bit<11> MaxTh = 28;
...
control MyIngress(inout headers hdr,
    ...
    register<bit<11>>(REGISTER_LENGTH) queue_r;
    register<bit<10>>(REGISTER_LENGTH) dp_r;
    ...
    action drop() {
         ...
         dropPkt.count((bit<32>) standard_metadata.ingress_port);
     }
    ...
    apply {
        ...
        if (standard_metadata.instance_type != PKT_INSTANCE_TYPE_INGRESS_RECIRC){
           ...
           if (hdr.ipv4.isValid()) {
                ...
                bit<11> queue_occupancy = (bit<11>)standard_metadata.enq_qdepth;
                queue_r.write(0,queue_occupancy);
            

                if (queue_occupancy <= MaxTh) {
             
                    bit<10> drop_random_temp;
                    dp_r.read(drop_random_temp,0);
                
                    bit<10> a = 1;
                    bit<10> drop_prob = a * 2;
                    dp_r.write(0,drop_prob);
                
                    bit<10> rand_val;
                    random(rand_val, 0, 1023);
...
control MyEgress(inout headers hdr,
    ...
    bit<11> qdepth = (bit<11>)standard_metadata.enq_qdepth;
    ...
    apply {
        ...
        // Add intrisic metadata in INT header 
        if (hdr.nodeCount.isValid()) {
            swtrace.apply();
        }

        // If minimum threshold was reached, recirculate pkt
        if (qdepth >= MinTh) {
            recirculate_packet();
        }
